...
let role_user = Data.readJSON(`${_path}/plugins/windoge-plugin/resources/dailyNote/json/`, "dispatch_time");

let path_url = ["dailyNote", "xiaoyao_Note"];
let path_img = ["background_image", "/icon/bg"];
let tempDataUrl = `${_path}/plugins/windoge-plugin/data/NoteTemp`
let tempData = {};
init()
...
export async function Note(e, {
	...
	let img_path = `./plugins/windoge-plugin/resources/dailyNote/${path_img[mb]}`;
	...
	for (let [user_id, cookie] of Object.entries(NoteCookie)) {
		...
		if (cookie.maxTime && cookie.maxTime > 0 && new Date().getTime() > cookie.maxTime - (160 - sendResin) * 8 *
			...
			if(isV3){
				...
				await Note(e, {render:await getPluginRender("windoge-plugin")});
			}
			await Note(e, getPluginRender("windoge-plugin"));
		}
	}
}
...
export async function pokeNote(e){
	...
	if(isV3){
		...
		let {getPluginRender} = await import(`file://${_path}/lib/render.js`);
		await Note(e, {render:await getPluginRender("windoge-plugin","poke")});
	}
}
...
const note_file = function(xiaoyao) {
	var urlFile = fs.readdirSync(`./plugins/windoge-plugin/resources/dailyNote/Template/`);
	var urlType = [];
	...
	// if(!xiaoyao){
	// 	var urlFileOne = fs.readdirSync(`./plugins/windoge-plugin/resources/dailyNote/background_image/`);
