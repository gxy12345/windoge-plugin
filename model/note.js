...
export default class note {
	...
	async getNote(cookie, uid, res,{render}){
		...
		await Common.render(`dailyNote/${path_url[mb]}`, {
			...
			urlType: encodeURIComponent(img_path.replace(
				/(\.\/plugins\/windoge-plugin\/resources\/|\/icon\/bg)/g, '')).replace(/%2F/g, "/"),
			resinMaxTime_mb2Day,
			resinMaxTime,
			resinMaxTime_mb2,
			remained_time,
	...
	note_file(xiaoyao) {
		let url1 = `./plugins/windoge-plugin/resources/dailyNote/Template/`
		let url3 = `./plugins/windoge-plugin/resources/dailyNote/background_image/`
